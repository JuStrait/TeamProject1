...
function initialize() {
    ...
    savedLocations = JSON.parse(localStorage.getItem("citiesSearched"));
    var lastSearch;
    ...
    if (savedLocations) {
        ...
        //try to geolocate, otherwise set city to Kansas City
        if (!navigator.geolocation) {
            //can't geolocate and no previous searches, so just give them one
            getCurrent("Kansas City");
        }
        else {
...
function success(position) {
    ...
    var queryURL = "https://api.openweathermap.org/data/2.5/weather?lat=" + lat + "&lon=" + lon + "&APPID=af8f0cf02906080babb5b0971511a2f6";
    $.ajax({
        url: queryURL,
        method: "GET"
...
function showPrevious() {
    ...
    if (savedLocations) {
        ...
        $("#pastCities").append(btns);
    }
}

function getCurrent(city) {
    var queryURL = "https://api.openweathermap.org/data/2.5/weather?q=" + city + "&APPID=af8f0cf02906080babb5b0971511a2f6&units=imperial";
    $.ajax({
        url: queryURL,
        method: "GET",
        error: function (){
            savedLocations.splice(savedLocations.indexOf(city), 1);
            localStorage.setItem("citiesSearched", JSON.stringify(savedLocations));
            initialize();
        }
    }).then(function (response) {
        //create the card
        var currCard = $("<div>").attr("class", "card bg-light");
        $("#cityWeather").append(currCard);
